...
const Header: FC = () => {
  ...
  const { selectedDate, setSelectedDate } = useSyncState()
    ...
    <nav className="w-full py-3 px-4 flex justify-between items-center lg:fixed z-50">
      ...
      <div className="hidden md:flex gap-3 items-center">
        ...
        >
          ...
          <div className="flex bg-gray-100 dark:bg-[#7d8082] text-gray-500 dark:text-[#f8fafc] text-xs rounded-md py-1 px-2 font-bold">
            ⌘G
          </div>
        </button>
      </div>
      <div className="flex md:hidden gap-4 items-center">
        <button
          aria-label="Date button"
          className="flex gap-4 items-center focus:outline-brand bg-gray-200 dark:bg-[#333338] md:bg-transparent px-3 py-1 rounded-lg md:py-o md:px-0"
          onClick={() => setOpen(!open)}
        >
